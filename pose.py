...
class Pose:
    ...
    def get_3d_pose(self) -> npt.NDArray[np.float32]:
        ...
        """
        return np.array(
            [self.x, self.y, self.z, *Rotation.as_euler(self.rotation, "xyz")]
        )
    ...
    def get_transformation_matrix(self) -> npt.NDArray[np.float32]:
        ...
        return np.block(
            [
                [self.rotation.as_matrix(), self.get_translation().reshape(3, 1)],
                [0, 0, 0, 1],
            ]
        )
    ...
    def from_transformation_matrix(matrix: npt.NDArray[np.float32]) -> Pose:
        ...
        return Pose(
            *matrix[0:3, 3],
            *Rotation.from_matrix(matrix[0:3, 0:3]).as_euler("xyz", degrees=False),
        )
    ...
    def __add__(self, other: Pose) -> Pose:
        ...
        """
        r_sum = Rotation.from_euler(
            "xyz",
            Rotation.as_euler(self.rotation, "xyz")
            + Rotation.as_euler(other.rotation, "xyz"),
        )
    ...
    def __mult__(self, other: float) -> Pose:
        ...
        """
        return Pose(
            *(self.get_translation() * other),
            *(Rotation.as_euler(self.rotation, "xyz") * other),
        )
